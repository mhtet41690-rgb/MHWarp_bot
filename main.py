...
def generate_hiddify_base64_link():
    try:
        setup_wgcf()
        reset_wgcf()
        ...
        with open("wgcf-account.toml", "r") as f:
            account_data = toml.load(f)
        client_id_b64 = account_data.get("client_id", "")
        res_bytes = list(base64.b64decode(client_id_b64)) if client_id_b64 else [0,0,0]
        reserved_str = f"{res_bytes[0]},{res_bytes[1]},{res_bytes[2]}"
        ...
        for line in lines:
            if line.startswith("PrivateKey"):
                priv_key = add_padding(line.split("=")[1].strip())
            elif line.startswith("PublicKey"):
                pub_key = add_padding(line.split("=")[1].strip())
            elif line.startswith("Address"):
                address = line.split("=")[1].strip()

        # 3. URL Encoding
        safe_priv = urllib.parse.quote(priv_key, safe='')
        safe_pub = urllib.parse.quote(pub_key, safe='')
        safe_addr = urllib.parse.quote(address, safe='')
        safe_reserved = urllib.parse.quote(reserved_str, safe='')

        # 4. Construct WireGuard URI
        full_uri = (
            f"wireguard://{safe_priv}@{FIXED_ENDPOINT}?"
            f"address={safe_addr}&"
            f"reserved={safe_reserved}&"
            f"publickey={safe_pub}&"
            f"mtu=1280#mhwarp"
        )

        # 5. Profile Title ထည့်သွင်းပြီး Base64 Encode လုပ်ခြင်း
        # သင်အလိုရှိတဲ့ format အတိုင်း စုစည်းပါတယ်
        final_string = "//profile-title: tg @mhwarp\n" + full_uri
        
        # String ကို bytes ပြောင်းပြီး Base64 လုပ်ခြင်း
    ...
    if text in ["⚡ Wireguard Key", "🧩 Hiddify Key"]:
        ...
        try:
            if text == "🧩 Hiddify Key":
                ...
                # ၁။ Config ဖတ်ခြင်းနှင့် လိုအပ်သော အချက်အလက်များ ထုတ်ယူခြင်း
                with open("wgcf-profile.conf", "r") as f:
                    lines = f.readlines()
                ...
                # ၂။ Key များ၏ အဆုံးတွင် %3D မဖြစ်မနေ ပါစေရန် ပြုလုပ်ခြင်း
                # '=' ကို အရင်ဖြုတ်ပြီးမှ '%3D' ကို တစ်ခါတည်း ပေါင်းထည့်ပါမည်
                safe_priv = priv_key.rstrip('=') + "%3D"
                safe_pub = pub_key.rstrip('=') + "%3D"

                # ၃။ Wireguard URI Format တည်ဆောက်ခြင်း
                wg_uri = (
                    f"wireguard://{FIXED_ENDPOINT}?"
                    f"private_key={safe_priv}&"
                    f"peer_public_key={safe_pub}&"
                    f"mtu=1280&"
                    f"address={address}#Mhwarp"
                )

                # ၄။ မူရင်း Config data (QR နှင့် File အတွက် - ပုံမှန်အတိုင်းထားပါသည်)
                data = "".join(lines).replace("engage.cloudflareclient.com:2408", FIXED_ENDPOINT)
                name = f"MH_{uuid.uuid4().hex[:8]}"
                
                # ၅။ File နှင့် QR ပို့ခြင်း
                with open(f"{name}.conf", "w") as f: f.write(data)
                qrcode.make(data).save(f"{name}.png")
                
                await update.message.reply_document(open(f"{name}.conf", "rb"), caption="📄 Wireguard Config File")
                await update.message.reply_photo(photo=open(f"{name}.png", "rb"), caption="📱 QR Code Scan")

                # ၆။ URI Link ပို့ပေးခြင်း
                copy_text = (
                    "📋 <b>Streisand Key (Only iOS)</b>\n\n"
                    f"<code>{wg_uri}</code>"
                )
                await update.message.reply_text(copy_text, parse_mode="HTML")
                
                # (ယခင်ပို့ထားသော URI Message ၏ အောက်တွင် ထည့်ရန်)

                # ၇။ Tutorial Message ပို့ခြင်း
                tutorial_msg = (
                    "📖 <b>အသုံးပြုနည်း လမ်းညွှန်</b>\n\n"
                    "၁။ အပေါ်က Streisand Key သည် iOS device တွတ်သာဖြစ်သည်။\n"
                    "💡Qr Code နှင့် File သည် Wireguard Vpn အတွတ်သာဖြစ်သည်။\n"
                    "၂။ <b>Wireguard</b> သို့မဟုတ် <b>Streisand</b> အသုံးပြုနည်းကို Channel တွင်တင်ထားပေးပါသည်။"
              
                )
                await update.message.reply_text(tutorial_msg, parse_mode="HTML")
                
                # Cleanup ဖိုင်များဖျက်ခြင်း
